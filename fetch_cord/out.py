...
def check_neofetch_scoop(default_config):
    return subprocess.run(
        [
            ...
            "--config=%s" % "none"
            if args.noconfig
            else (args.config_path if args.config_path else (default_config)),
        ],
        encoding="utf-8",
...
def check_neofetchwin():
    return subprocess.run(
        ["neofetch", "--noart"], check=True, encoding="utf-8", stdout=subprocess.PIPE
    ).stdout
...
def neofetch(loop):
    ...
    if os.name == "nt":
        ...
        if (
            loop == 0
            and flatpak_discord_path
            and not package_path
            and not manual_install_path
        ):
            XDG_Symlink(home)
        try:
        ...
        for i in range(len(lines)):
            ...
            if neofetchwin:
                if line.find(nvidiagpu) != -1:
                    gpuline.append(
                        "".join(["GPU:", line[line.find(nvidiagpu) :]]).rstrip("\n")
                    )
